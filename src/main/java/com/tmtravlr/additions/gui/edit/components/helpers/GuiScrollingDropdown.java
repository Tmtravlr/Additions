...
public class GuiScrollingDropdown<T> extends GuiScrollingList {
	...
	public GuiDropdownListInput parentInput;
	public List<T> selections;
	public List<T> selectionDisplay;
	public String prevFilter = "";
	
	public GuiScrollingDropdown(GuiDropdownListInput parentInput, List<T> selections, int height, boolean above) {
		super(parentInput.parent.mc, parentInput.selectedText.width, height,
				...
				parentInput.selectedText.x, 18, parentInput.parent.width, parentInput.parent.height);
	...
	protected void drawSlot(int slot, int right, int top, int slotBuffer, Tessellator tess) {
		if (!this.selectionDisplay.isEmpty()) {
			...
			if (toDisplay != null) {
				this.parentInput.parent.getFontRenderer().drawString(this.parentInput.getSelectionName(toDisplay), this.left + 5, top + 2, 0xFFFFFF);
			}
		} else {
			this.parentInput.parent.getFontRenderer().drawString(I18n.format("gui.dropdown.nothingToShow"), this.left + 5, top + 2, 0x888888);
		}
	}
