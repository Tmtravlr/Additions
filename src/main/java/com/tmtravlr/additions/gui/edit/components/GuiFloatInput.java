...
/**
 * Extension of the vanilla text field, for a number input
 * @author Rebeca Rey (Tmtravlr)
 * @date August 2017
 */
public class GuiFloatInput extends GuiTextField implements IGuiEditComponent {
	...
	public GuiFloatInput(String label, GuiEdit parentScreen, boolean allowNegative) {
	...
	public void drawInList(int x, int y, int right, int mouseX, int mouseY) {
	    ...
	    this.parent.drawTexturedModalRect(this.x + this.width, this.y - 1, 163, 64, 11, 22);
	    
	    if (this.clickUpTime > 0) {
	    	this.parent.drawTexturedModalRect(buttonX, buttonUpY, 174, 74, 9, 10);
	    } else {
	    	this.parent.drawTexturedModalRect(buttonX, buttonUpY, 174, 64, 9, 10);
	    }
	    
	    if (this.clickDownTime > 0) {
	    	this.parent.drawTexturedModalRect(buttonX, buttonDownY, 183, 74, 9, 10);
	    } else {
	    	this.parent.drawTexturedModalRect(buttonX, buttonDownY, 183, 64, 9, 10);
	    }		
