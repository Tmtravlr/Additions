...
public class GuiScrollingComponentList<T extends IGuiEditComponent> extends GuiScrollingListAnyHeight {
	...
	private GuiComponentList<T> parent;
	public ArrayList<IGuiEditComponent> displayComponents = new ArrayList<>();
	
	public GuiScrollingComponentList(GuiComponentList parent, int x, int y, int width, int height) {
		super(parent.parent.mc, width, height, y, x, parent.parent.width, parent.parent.height);
		this.parent = parent;
	}
	...
	protected void elementClicked(int slot, boolean doubleClick) {
		IGuiEditComponent component = displayComponents.get(slot);
		if (component == this.parent.addNewComponent) {
			//Add new component
			this.parent.addNewComponent();
		} else if (component == this.parent.insertNewComponent) {
			//Insert a component here
			int index = slot / 2;
			this.parent.insertNewComponent(index);
		} else {
			if (this.mouseX >= this.left && this.mouseX < this.left + REMOVE_COMPONENT_WIDTH) {
				//Remove component
				int index = slot / 2;
				this.parent.removeComponent(index);
			}
		}
	}
	...
	protected void drawSlot(int slot, int right, int top, int buffer, Tessellator tess) {
		IGuiEditComponent component = this.displayComponents.get(slot);
		...
		if (component == this.parent.addNewComponent) {
			...
			if (this.mouseX >= this.left && this.mouseX <= right && this.mouseY >= top && this.mouseY < top + height ) {
				this.parent.parent.drawRect(this.left + 1, top, right - 1, top + height, 0xFF102123);
				this.parent.parent.drawRect(this.left + 3, top + 2, right - 3, top + height - 2, 0xFF1C393C);
				this.parent.parent.drawRect(this.left + 5, top + 4, right - 5, top + height - 4, 0xFF2D5C60);
			}
			
			this.parent.parent.mc.getTextureManager().bindTexture(GUI_TEXTURES);
		    GlStateManager.color(255.0F, 255.0F, 255.0F, 255.0F);
		    
			this.parent.parent.drawTexturedModalRect(this.left + (right - this.left)/2 - 7, top + height/2 - 7, 73, 64, 13, 13);
		} else if (component == this.parent.insertNewComponent) {
			
			if (this.mouseX >= this.left && this.mouseX <= right && this.mouseY >= top && this.mouseY < top + height ) {
				this.parent.parent.drawRect(this.left + 1, top, right - 1, top + height, 0xFF102123);
				this.parent.parent.drawRect(this.left + 2, top + 1, right - 2, top + height - 1, 0xFF1C393C);
				this.parent.parent.drawRect(this.left + 3, top + 2, right - 3, top + height - 2, 0xFF2D5C60);
			...
			if (this.mouseX >= this.left && this.mouseX <= this.left + REMOVE_COMPONENT_WIDTH && this.mouseY >= top && this.mouseY < top + height ) {
				this.parent.parent.drawRect(this.left + 1, top, this.left + REMOVE_COMPONENT_WIDTH - 2, top + height, 0xFF210F0F);
				this.parent.parent.drawRect(this.left + 2, top + 1, this.left + REMOVE_COMPONENT_WIDTH - 3, top + height - 1, 0xFF351919);
				this.parent.parent.drawRect(this.left + 3, top + 2, this.left + REMOVE_COMPONENT_WIDTH - 4, top + height - 2, 0xFF602D2D);
			}
			...
			this.parent.parent.drawTexturedModalRect(this.left + REMOVE_COMPONENT_WIDTH/2 - 7, top + height/2 - 7, 60, 64, 13, 13);
		}
	}
	...
	public void handleMouseInput(int mouseX, int mouseY) throws IOException {
		...
		for (IGuiEditComponent component : this.displayComponents) {
