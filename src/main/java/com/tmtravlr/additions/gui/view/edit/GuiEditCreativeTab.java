...
public class GuiEditCreativeTab extends GuiEdit {
	...
	public void initComponents() {
		...
		this.idInput = new GuiComponentStringInput("gui.edit.creativeTab.id.label", this);
		if (this.isNew) {
			this.idInput.setRequired();
			this.idInput.setInfo(new TextComponentTranslation("gui.edit.id.info"));
			this.idInput.setValidator(new Predicate<String>() {
		...
		this.nameInput = new GuiComponentStringInput("gui.edit.creativeTab.name.label", this);
		...
		this.hasSearchBarInput = new GuiComponentBooleanInput("gui.edit.creativeTab.hasSearchBar.label", this);
		if (!this.isNew) {
		...
		this.itemListInput = new GuiComponentListInput<GuiComponentItemStackInput>("gui.edit.creativeTab.items.label", this) {
		...
		if (!this.isNew) {
			this.oldTab.getDisplayItems().forEach(toAdd->{
				GuiComponentItemStackInput input = new GuiComponentItemStackInput("", this);
				input.disableCount();
				input.setDefaultItemStack(toAdd);
				this.itemListInput.addComponent(input);
			});
		}
	...
	public void saveObject() {
		...
		if (this.idInput.getText().isEmpty()) {
			this.mc.displayGuiScreen(new GuiMessagePopup(this, I18n.format("gui.edit.creativeTab.problem.title"), new TextComponentTranslation("gui.edit.problem.noId", name), I18n.format("gui.buttons.back")));
			return;
		}
		
		if (displayName.isEmpty()) {
			this.mc.displayGuiScreen(new GuiMessagePopup(this, I18n.format("gui.edit.creativeTab.problem.title"), new TextComponentTranslation("gui.edit.problem.noName", name), I18n.format("gui.buttons.back")));
			return;
		}
		
		if (this.itemListInput.getComponents().isEmpty()) {
			this.mc.displayGuiScreen(new GuiMessagePopup(this, I18n.format("gui.edit.creativeTab.problem.title"), new TextComponentTranslation("gui.edit.creativeTab.problem.noItems", name), I18n.format("gui.buttons.back")));
			return;
		}
		
		if (this.isNew && AdditionTypeCreativeTab.INSTANCE.hasCreativeTabWithId(this.addon, name)) {
			this.mc.displayGuiScreen(new GuiMessagePopup(this, I18n.format("gui.edit.creativeTab.problem.title"), new TextComponentTranslation("gui.edit.creativeTab.problem.duplicate", name), I18n.format("gui.buttons.back")));
			return;
		}
		
		CreativeTabAdded tab = new CreativeTabAdded(displayName);
		tab.setId(name);
		...
		this.mc.displayGuiScreen(new GuiMessagePopup(this.parentScreen, I18n.format("gui.edit.creativeTab.success.title"), new TextComponentTranslation("gui.edit.creativeTab.success.message"), I18n.format("gui.buttons.continue")));
	}
